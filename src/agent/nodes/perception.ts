...
export async function perceptionNode(
  ...
  console.log(
    `[Perception] Starting perception node - Iteration ${state.iteration_count}`
  );
  ...
  try {
    ...
    console.log(
      `[Perception] Screenshot captured: ${screenshot.dimensions.width}x${screenshot.dimensions.height}`
    );
    ...
    console.log(
      `[Perception] Successfully detected ${perceptionResult.elements.length} UI elements`
    );

    // Log detected elements for debugging
    perceptionResult.elements.forEach((element, index) => {
      console.log(
        `[Perception] Element ${index}: ${element.type} - "${element.content}" at [${element.bbox.join(", ")}]`
      );
    });
