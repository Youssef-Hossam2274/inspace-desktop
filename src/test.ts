...
console.log("Resolved perceptionAPI import:", import.meta.resolve("./services/perceptionAPI.ts"));
...
async function testIndividualComponents() {
  ...
  try {
    ...
    try {
      ...
      console.error("❌ Screenshot/Perception test failed:", error instanceof Error ? error.message : error);
    }
    ...
    try {
      ...
      const testContext = {
        user_prompt: "You are in vscode, i want you to press inside the terminal, then type 'npm run dev' and press enter",
        current_elements: [],
        iteration_count: 1,
        previous_actions: [],
      ...
      console.error("❌ LLM API test failed:", error instanceof Error ? error.message : error);
    }
